    ...
    return (
        <div ref={ref} className="group md:h-full">
            <div className="appear-container hover:translate-x-[1rem] hover:translate-y-[-1rem] hover:shadow-[-24px_30px_0px_-3px_#000000] shadow-[0px_0px_0px_0px_#000000] transition-all duration-300 h-full">
                <a href={link} className={`group/card relative block w-full md:h-full dark:invert overflow-hidden sm:h-100 ${isVisible ? 'appear-up' : 'opacity-0'}`} style={{ animationDelay: `${delay}ms` }}>
                    {image && (
            ...
            <div className="appear-container">
                <h2 className={`text-4xl mt-3 mb-2 dark:invert ${isVisible ? 'appear-up' : 'opacity-0'}`} style={{ fontFamily: 'Secondary-Font, Consolas', animationDelay: `${delay + 400}ms` }}>{title}</h2>
            </div>
            <div className="appear-container">
                <p className={`mt-0 font-thin dark:invert ${isVisible ? 'appear-up' : 'opacity-0'}`} style={{ animationDelay: `${delay + 600}ms` }}>{description}</p>
            </div>
        </div>
    )
}